...
- (void) dialogDidSucceed:(NSURL*)url {
  ...
  if (expTime != nil) {
	...
	//storing defaults so the user isn't prompted every time to login
	[[NSUserDefaults standardUserDefaults] setObject:token forKey:@"access_token"];
	[[NSUserDefaults standardUserDefaults] setObject:expirationDate forKey:@"exp_date"];
...
/* New Overriding from the base class*/
- (void) dialogRequestingCredentials {
	
	//added we don't want to display the dialog unless they need to login
	self.hidden = NO;
	
	// new facebook login dialog delegate method
	if ([_loginDelegate respondsToSelector:@selector(fbRequestingCredentials)])
	   [_loginDelegate fbRequestingCredentials];
}
