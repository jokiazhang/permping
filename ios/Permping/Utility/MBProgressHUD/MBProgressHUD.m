...
- (void)setMBLabelText:(NSString *)newText {
    [self performSelectorOnMainThread:@selector(updateLabelText:) withObject:newText waitUntilDone:YES];
    [self performSelectorOnMainThread:@selector(setNeedsLayout) withObject:nil waitUntilDone:NO];
    [self performSelectorOnMainThread:@selector(setNeedsDisplay) withObject:nil waitUntilDone:NO];
}
...
- (void)layoutSubviews {
    ...
    indFrame.origin.x = ((frame.size.width - indFrame.size.width) / 2) + self.xOffset;//floor((frame.size.width - indFrame.size.width) / 2) + self.xOffset;
    indFrame.origin.y = ((frame.size.height - indFrame.size.height) / 2) + self.yOffset;//floor((frame.size.height - indFrame.size.height) / 2) + self.yOffset;
    ...
    if (nil != self.labelText) {
        ...
        indFrame.origin.y -= (lHeight / 2 + PADDING / 2);//(floor(lHeight / 2 + PADDING / 2));
        indicator.frame = indFrame;
...
//                                   lWidth, lHeight);
        CGRect lFrame = CGRectMake((frame.size.width - lWidth) / 2 + xOffset,
                                   (indFrame.origin.y + indFrame.size.height + PADDING),
                                   lWidth, lHeight);
        label.frame = lFrame;
