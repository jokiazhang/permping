...
/**
 *	Get cateogies of library which are supported in app
 */
+ (NSDictionary *)getSupportedDeviceCategory
{
	...
	return [configurationDict objectForKey:CLOUD_SUPPORT_DEVICE_CATEGORY_KEY];
}

/**
 *	Check the supported recommendation in configuration file
 */
+ (BOOL)isSupportedRecommendation
{
	...
	if ([configurationDict objectForKey:CLOUD_SUPPORT_RECOMMENDATION_KEY] != nil)
	{
		return [[configurationDict objectForKey:CLOUD_SUPPORT_RECOMMENDATION_KEY] boolValue];
	}
	return NO;
}
...
+ (BOOL)isSupportedLocalLibrary
{
	...
	if ([Utility isOS4Device] && [[configurationDict objectForKey:CLOUD_SUPPORT_LOCAL_CONTENT_KEY] boolValue] == YES) 
	{
		return YES;
	}
...
+ (BOOL)isSupportedRovi
{
	...
	if ([[configurationDict objectForKey:CLOUD_SUPPORT_ROVI] boolValue] == YES) 
	{
		return YES;
	}
...
+ (BOOL)isSupportedSharing
{
	...
	if ([configurationDict objectForKey:CLOUD_SUPPORT_SHARINGITEM_KEY] != nil)
	{
		return [[configurationDict objectForKey:CLOUD_SUPPORT_SHARINGITEM_KEY] boolValue];
	}
	return NO;
}
