...
- (void) publishPostWithDelegate:(id) _delegate {
	...
	if (!loggedIn) {
		[[FBRequestWrapper defaultManager] FBSessionBegin:self showDialog:YES];
	}
	else {
...
- (void) showLoginViewWithDelegate:(id) _delegate {
...
- (void) checkLoggedIn {
    isPost = NO;
    [[FBRequestWrapper defaultManager] FBSessionBegin:self showDialog:NO];
}
...
- (void)fbDidNotLogin:(BOOL)cancelled {
    ...
    if ([self.delegate respondsToSelector:@selector(didNotLogin:)] && !cancelled) {
        [self.delegate didNotLogin:self];
    }
