...
- (void)authorizeWithFBAppAuth:(BOOL)tryFBAppAuth
                    safariAuth:(BOOL)trySafariAuth showDialog:(BOOL)showDialog {
...
- (void)authorize:(NSString *)application_id
  ...
  [self authorizeWithFBAppAuth:NO safariAuth:NO showDialog:YES];
}

- (void)authorize:(NSString *)application_id
      permissions:(NSArray *)permissions
         delegate:(id<FBSessionDelegate>)delegate showDialog:(BOOL)showDialog{
    [_appId release];
    _appId = [application_id copy];
    ...
    //changed to NO so the modal dialog appears
    [self authorizeWithFBAppAuth:NO safariAuth:NO showDialog:showDialog];
}
...
- (BOOL)handleOpenURL:(NSURL *)url {
  ...
  if (!accessToken) {
    ...
    if (errorReason && [errorReason isEqualToString:@"service_disabled_use_browser"]) {
      [self authorizeWithFBAppAuth:NO safariAuth:YES showDialog:YES];
      return YES;
    }
