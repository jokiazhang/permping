...
- (id) initWithEngine: (SA_OAuthTwitterEngine *) engine andOrientation:(UIInterfaceOrientation)theOrientation {
	if (self = [super init]) {
        ...
        // use loginView
        NSLog(@"_engine URL: %@", [[_engine.authorizeURLRequest URL] absoluteString]);    
        loginView = [[TwitterLoginDialog alloc] initWithRequest:_engine.authorizeURLRequest];
...
- (void) denied {
    ...
    //[self performSelector:@selector(dismiss:) withObject:(id) kCFBooleanTrue afterDelay:1.0];
    [loginView performSelector:@selector(dismiss:) withObject:(id) kCFBooleanTrue afterDelay:1.0];
}

- (void) gotPin: (NSString *) pin {
    NSLog(@"gotPin pin: %@", pin);
	_engine.pin = pin;
	...
	//[self performSelector: @selector(dismissModalViewControllerAnimated:) withObject: (id) kCFBooleanTrue afterDelay: 0.5];
    //[self performSelector:@selector(dismiss:) withObject:(id)kCFBooleanTrue afterDelay:0.5];
    [loginView performSelector:@selector(dismiss:) withObject:(id)kCFBooleanTrue afterDelay:0.5];
}
...
- (void) cancel: (id) sender {
    ...
    //[self performSelector:@selector(dismiss:) withObject:(id)kCFBooleanTrue afterDelay:0];
    [loginView performSelector:@selector(dismiss:) withObject:(id)kCFBooleanTrue afterDelay:0];
}
...
- (void) webViewDidFinishLoad: (UIWebView *) webView {
    ...
    */
    [loginView stopWaitingAnimating];
}
...
- (void)webView:(UIWebView *)webView didFailLoadWithError:(NSError *)error {
    NSLog(@"twitter view did fail load with error: %@", [error localizedDescription]);
    
    UIAlertView *alerView = [[UIAlertView alloc] initWithTitle:@"Error" message:@"Cannot connect to Twitter" delegate:nil cancelButtonTitle:@"OK" otherButtonTitles: nil];
    [alerView show];
    [alerView release];
    
    //[self dismissModalViewControllerAnimated:YES];
    
    [loginView dismiss:YES];
}
